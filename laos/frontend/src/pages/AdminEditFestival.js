...
const AdminEditFestival = () => {
  ...
  useEffect(() => {
    const fetchFestival = async () => {
      try {
        setLoading(true);
        const response = await adminService.getFestivalById(id);
        // Response: ApiResponse<FestivalResponse> → data.data
        ...
        setError('Lỗi khi tải dữ liệu lễ hội');
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await adminService.updateFestival(id, fd);
      setSuccess('Cập nhật lễ hội thành công!');
      setTimeout(() => navigate('/admin/festivals'), 1500);
    } catch (err) {
      setError(err.response?.data?.message || err.message || 'Lỗi khi cập nhật lễ hội');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <AdminLayout title="Chỉnh sửa lễ hội">
