...
const AdminFestivals = () => {
  ...
  const fetchFestivals = useCallback(async () => {
    try {
      ...
      const response = await adminService.getAllFestivals(params);
      // Response: ApiResponse<PageResponse<FestivalResponse>> → data.data.content
      ...
      setError('Lỗi khi tải danh sách lễ hội');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmDelete = async (id) => {
    try {
      ...
      setError(err.response?.data?.message || 'Lỗi khi xóa lễ hội');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout title="Quản lý lễ hội">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Lễ hội</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tỉnh/Thành</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Ngày bắt đầu</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Ngày kết thúc</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {festivals.map((festival) => (
                  <tr key={festival.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-500">{festival.province || '—'}</td>
                    <td className="px-4 py-3 text-gray-500">{formatDate(festival.startDate)}</td>
                    <td className="px-4 py-3 text-gray-500">{formatDate(festival.endDate)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${festival.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                        {festival.status === 'ACTIVE' ? 'Hoạt động' : 'Không hoạt động'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{festival.viewCount ?? 0}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => navigate(`/admin/festivals/edit/${festival.id}`)}
                        ...
                        <button
                          onClick={() => handleDeleteClick(festival.id)}
        ...
        {!loading && festivals.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-4">{provinceFilter ? 'Không tìm thấy lễ hội nào' : 'Chưa có lễ hội nào'}</p>
            {!provinceFilter && (
              <button
                onClick={() => navigate('/admin/festivals/create')}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-gray-500 text-sm text-center mb-6">Bạn có chắc chắn muốn xóa lễ hội này? Hành động này không thể hoàn tác.</p>
